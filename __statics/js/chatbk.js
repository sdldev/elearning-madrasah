	...
	if(messageTxt!=''){
		//console.log(message);
 		DisplayMessage(messageTxt);
				...
				if (document.getElementById('anonim').checked) {
					var anonim =1;
				}else{
					var anonim =0;
				}
				...
				$.ajax({
						  dataType : "json",
						  type : 'post',
						  data : {messageTxt : messageTxt, layanan : layanan,anonim:anonim},
						  
						  url: base_url+'layananbk/send_text_message',
						  success:function(data)
						  {
  							GetChatHistory(layanan)		 
						  },
						  error: function (jqXHR, status, err) {
 							 // alert('Local error callback');
						  }
...
function GetChatHistory(receiver_id){
				$.ajax({
						  //dataType : "json",
  						  url: base_url+'layananbk/get_chat_history_by_vendor?receiver_id='+receiver_id,
						  success:function(data)
						  {
  							$('#dumppy').html(data);
							ScrollDown();	 
